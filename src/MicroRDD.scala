...
class MicroRDD(operator: Int, f: Any => Any = null, f2: (Any, Any) => Any = null, f3: Any => Array[Any] = null, numPartitions: Int = -1) extends Serializable {
  ...
  def computeAncestralSubgraphHash(curId: Int, isRoot: Boolean): Int = {
    if (isRoot) return -1
    val node = QaaD.microRddDag(curId)
    val parentHashes = node.parentList
      .map(parentId => QaaD.microRddDag(parentId).hashVal)
      .sorted
    val nodeHash = node.operation.operator.toString + "," + (if (f != null) f.hashCode.toString else "-1") + "," + (if (f2 != null) f2.hashCode.toString else "-1") + "," + (if (f3 != null) f3.hashCode.toString else "-1") + "," + paramMap.toString
    (nodeHash + "," + parentHashes.mkString(",")).hashCode
  }
  ...
  def removeChild(childMicroRDDId: MicroRDDId): Unit = {
    this.childList = this.childList.filter(x => x != childMicroRDDId)
  }
  ...
  def qMap(f: Any => Any)(implicit appId: Int, appParamMap: Map[String, Any]): MicroRDD = {
    ...
    childMicroRdd.appId = appId
    childMicroRdd.paramMap = appParamMap
    childMicroRdd.hashVal = childMicroRdd.computeAncestralSubgraphHash(childMicroRdd.id, childMicroRdd.parentList.size == 0)
    if (QaaD.microRddIdByHashVal.contains(childMicroRdd.hashVal)) {
      QaaD.microRddDag.remove(childMicroRdd.id)
      this.removeChild(childMicroRdd.id)
      QaaD.microRddDag(QaaD.microRddIdByHashVal(childMicroRdd.hashVal))
    } else {
      QaaD.microRddIdByHashVal.put(childMicroRdd.hashVal, childMicroRdd.id)
      childMicroRdd
    }
  }

  def qFlatMap(f: Any => Array[Any])(implicit appId: Int, appParamMap: Map[String, Any]): MicroRDD = {
  ...
  def qFilter(f: Any => Any)(implicit appId: Int, appParamMap: Map[String, Any]): MicroRDD = {
  ...
  def qSample(frac: Float)(implicit appId: Int, appParamMap: Map[String, Any]): MicroRDD = {
  ...
  def qJoin(other: MicroRDD)(implicit appId: Int, appParamMap: Map[String, Any]): MicroRDD = {
    ...
    if (QaaD.microRddIdByHashVal.contains(childMicroRdd.hashVal)) {
      QaaD.microRddDag.remove(childMicroRdd.id)
      this.removeChild(childMicroRdd.id)
      other.removeChild(childMicroRdd.id)
      QaaD.microRddDag(QaaD.microRddIdByHashVal(childMicroRdd.hashVal))
    } else {
      QaaD.microRddIdByHashVal.put(childMicroRdd.hashVal, childMicroRdd.id)
      childMicroRdd
  ...
  def qReduceByKey(f: (Any, Any) => Any, numPartitions: Int = -1)(implicit appId: Int, appParamMap: Map[String, Any]): MicroRDD = {
  ...
  def qCount(): MicroRDD = {
    implicit val appId = -1
    implicit val appParamMap = Map[String, Any]()
...
def qTextFile(inputName: String): MicroRDD = {
  if (!QaaD.microRddIdByInputName.contains(inputName)) {
    ...
    QaaD.microRddIdByHashVal.put(microRdd.hashVal, microRdd.id)
    microRdd
  } else {
...
class Scheduler() extends Serializable {
  ...
  def recursiveProceed(resultRecord: (MicroRDDId, Any)): Array[(MicroRDDId, Any)] = {
                var containCur = false
                if (QaaD.microRddDag(resultRecord._1).childList.size == 0) {
			Array[(MicroRDDId, Any)](resultRecord)
                } else {
		QaaD.microRddDag(resultRecord._1).childList.flatMap { childMicroRddId =>
...
*/
				...
				inputRDD.filter(r => parentMicroRddIds.contains(r._1))
					.mapPartitions { partition: Iterator[(MicroRDDId, Any)] =>
						proceed(partition.flatMap { case record: (MicroRDDId, Any) =>
							...
							val value = record._2
							QaaD.microRddDag(microRddId).childList
                //.filter(c => getType(QaaD.microRddDag(c).operation.operator) == NARROW_DEPENDENCY)
                .flatMap { childMicroRddId: MicroRDDId =>
  ...
  def updateDecendants(microRddId: MicroRDDId): Unit = {
    ...
    QaaD.microRddDag(curMicroRddId).childList.foreach { childMicroRddId =>
      val operatorType = getType(QaaD.microRddDag(childMicroRddId).operation.operator)
      if (QaaD.microRddDag(childMicroRddId).parentList.size == 1 &&
	operatorType == NARROW_DEPENDENCY) {
        updateMicroRdd(childMicroRddId, QaaD.rddArr.result().size - 1)
        updateDecendants(childMicroRddId)
      }
    }
  }
/*
...
*/
  ...
  def run(): Unit = {
    println("# of vertices: " + QaaD.microRddDag.size)
