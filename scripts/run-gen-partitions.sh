...
dataset=$1
num_rows=$2
input_path=/root/qbatcher/scripts/input/${dataset}-${num_rows}.txt
num_partitions=$3
num_queries=1
/root/qbatcher/scripts/set.sh ori-jars
if [ ${dataset} == bra ]; then
  for i in {0..10}; do
    hdfs_dir=/root/qbatcher/datasets/synthetic-brazilian-ecommerce/num-rows-${num_rows}/
    ...
    /root/dev/spark-3.2.1-bin-hadoop2.7/bin/spark-shell \
      ...
      -i /root/qbatcher/src/SetupForGenDataset.scala \
      -i /root/qbatcher/querysets/brazilian-ecommerce/load.scala \
      -i /root/qbatcher/src/GenDataset-bra.scala > /dev/null # 2>&1
    dir_count=$(/root/dev/hadoop-2.7.7/bin/hdfs dfs -ls ${hdfs_dir} | wc -l)
    if [[ "${dir_count}" -eq "9" ]]; then
      break
    else
      /root/qbatcher/scripts/clear-tmp-files.sh
    fi
  done
else
  for i in {0..10}; do
    hdfs_dir=/root/qbatcher/datasets/synthetic-ebay/num-rows-${num_rows}/
    /root/dev/hadoop-2.7.7/bin/hdfs dfs -rm -r -f ${hdfs_dir}/*
    /root/dev/hadoop-2.7.7/bin/hdfs dfs -mkdir -p ${hdfs_dir}
    /root/dev/spark-3.2.1-bin-hadoop2.7/bin/spark-shell \
      --master yarn \
      ...
      -i /root/qbatcher/src/Operation.scala \
      -i /root/qbatcher/src/SetupForGenDataset.scala \
      -i /root/qbatcher/querysets/ebay/load.scala \
      -i /root/qbatcher/src/GenDataset-ebay.scala > /dev/null 2>&1
    dir_count=$(/root/dev/hadoop-2.7.7/bin/hdfs dfs -ls ${hdfs_dir} | wc -l)
